locals {
  ...
  cluster_service_cidr              = coalescelist(aws_eks_cluster.this[*].kubernetes_network_config[0].service_ipv4_cidr, [""])[0]
  ...
  launch_configuration_userdata_rendered = [
    for index in range(var.create_eks ? local.worker_group_launch_configuration_count : 0) : templatefile(
      lookup(
        ...
        : lookup(var.worker_groups[index], "ami_type", local.workers_group_defaults["ami_type"]) == "al2023"
  ...
  launch_template_userdata_rendered = [
    for index in range(var.create_eks ? local.worker_group_launch_template_count : 0) : templatefile(
      lookup(
        ...
        ? "${path.module}/templates/userdata_windows.tpl"
        : lookup(var.worker_groups_launch_template[index], "ami_type", local.workers_group_defaults["ami_type"]) == "al2023"
        ? "${path.module}/templates/userdata_al2023.tpl"
        : "${path.module}/templates/userdata.sh.tpl"
      ),
      merge({
